                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped table-hover" id="datatable2" >
                                        ...
                                        @foreach($product as $items)
                                            <tr>
                                                <td>{{$i}}</td>
                                                <td>{{$items->produk_title}}</td>
                                                <td>{{$items->produk_version}}</td>
                                                <td>{{$items->produk_type}}</td>
                                                <td>{{$items->name}}</td>
                                                <td>{{$items->created_at}}</td>
                                                <td>

                                                    @if($items->produk_status ==1)
                                                        <span class='label label-system'>Available</span>
                                                    @elseif($items->produk_status==2)
                                                ...
                                                <td>
                                                    <div class="btn-group">
                                                        @if($items->produk_type == 'Freemium')
                                                            @if($items->produk_status ==1)
                                                                <a href="{{URL::route('dashboard-moderator-manageproduct-product-list-banned',$items->id_produk)}}" onclick="return confirm('Are you sure banned this product ?')"  class="btn btn-danger" title="Banned"><i class="fa fa-ban"></i></a>
                                                            @else
                                                                <a href="{{URL::route('dashboard-moderator-manageproduct-product-list-available',$items->id_produk)}}" onclick="return confirm('Are you sure approve this product ?')"  class="btn btn-system" title="Visible"><i class="fa fa fa-check-circle"></i></a>
                                                            @endif
                                                            <a href="{{URL::route('dashboard-moderator-manageproduct-product-archive',$items->id_produk)}}" class="btn btn-info" title="Product Archive"><i class="fa fa-archive"></i></a>
                                                        @else
                                                            <a href="#" onclick="return confirm('Are you sure banned this product ?')"  class="btn btn-danger" title="Banned"><i class="fa fa-ban"></i></a>
                                                        @endif
                                                    </div>
                                                </td>
                                            </tr>
